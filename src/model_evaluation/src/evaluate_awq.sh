...
# Note:   Script to evaluate the performance of the AWQ models with lm-eval suite.
...
for MODEL in ${MODELS[@]};do
    CUDA_VISIBLE_DEVICES=0 lm-eval \
        --model vllm \
        --model_args pretrained=${MODEL_DIR/${MODEL}-AWQ/,tensor_parallel_size=1,dtype=auto,gpu_memory_utilization=0.8,data_parallel_size=1 \
        --tasks winogrande,arc_easy,arc_challenge,truthfulqa,hellaswag \
        --batch_size auto |& tee ${MODEL}-AWQ.log
done
