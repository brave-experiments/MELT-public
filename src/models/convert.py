...
from convert_utils.llama_utils import (LLAMA_CPP_HOME, convert_ggml,
                                       convert_yaml_to_json_config,
                                       llama_change_model_config_eos,
                                       llama_translate_config_to_model_config)
from convert_utils.mlc_utils import (MLC_HOME, convert_mlc,
...
def parse_args():
    ...
    args.add_argument(
        "--only-config", action="store_true", help="Produce only the config file"
    )
    args.add_argument(
        "--ignore-eos",
        action="store_true",
...
def validate_args(args):
    if args.backend == "mlc":
        ...
        if not args.config:
            raise ValueError("MLC requires a config file to be specified.")
        ...
        try:
            from convert_utils.awq_utils import (decode_quant_method,
                                                 quantize_awq)
    ...
    if args.backend == "mlc":
        if args.only_config:
            raise NotImplementedError(
                "Only config is only supported for ggml.")
        ...
        if chat_config_path and args.ignore_eos:
            mlc_change_model_template_eos(
                chat_config_path)  # Requires custom backend
        ...
        if args.ignore_eos:  # revert it back for idempotence
            llama_change_model_config_eos(args.model, previous_eos)
        llama_translate_config_to_model_config(
            args.config, args.output_dir, ignore_eos=args.ignore_eos
        )
        convert_yaml_to_json_config(
            args.config, os.path.join(args.output_dir, "model_config.json")
        )
    elif (
        args.backend == "awq"
    ):  # This was uttimately not used in paper, unless TheBloke repo did not include model.
        from convert_utils.awq_utils import decode_quant_method, quantize_awq
