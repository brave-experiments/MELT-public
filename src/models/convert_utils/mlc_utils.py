...
def mlc_translate_config_to_model_config(config_path, chat_config_path):
    ...
    model_config["temperature"] = config["sampling"].get(
        "temperature", model_config["temperature"]
    )
    model_config["repetition_penalty"] = config["sampling"].get(
        "repetition_penalty", model_config["repetition_penalty"]
    )
    model_config["top_p"] = config["sampling"].get(
        "top_p", model_config["top_p"])

    model_config["mean_gen_len"] = config["generation"].get(
        "mean_gen_len", model_config["mean_gen_len"]
    )
    model_config["max_gen_len"] = config["generation"].get(
        "max_gen_len", model_config["max_gen_len"]
    )
    if "max_window_size" in model_config:
        model_config["max_window_size"] = config["generation"].get(
            "max_window_size", model_config["max_window_size"]
        )
    model_config["vocab_size"] = config["generation"].get(
        "vocab_size", model_config["vocab_size"]
    )

    model_config["conv_template"] = config["prompt"].get(
        "conv_template", model_config["conv_template"]
    )
...
def convert_mlc(model_dir, args):
    ...
    chat_config = os.path.join(
        args.output_dir, "**", "params", "mlc-chat-config.json")
