...
def convert_ggml(model_dir, args):
    ...
    if "tinyllama" in model_name:
        ...
        args_list = ["python", exec_path,
                     "--outfile", gguf_model, model_filename]
    elif "starcoder" in model_name:
        exec_path = os.path.join(
            LLAMA_CPP_HOME, "convert-starcoder-hf-to-gguf.py")
        args_list = ["python", exec_path,
                     model_dir, "0", "--outfile", gguf_model]
    elif "zephyr-3b" in model_name or (
        model_name in ["google_gemma-2b-it", "google_gemma-7b-it"]
    ):
        ...
        args_list = ["python", exec_path, "--outfile", gguf_model, model_dir]
    ...
    if model_name in ["google_gemma-2b", "google_gemma-7b"]:
        ...
        shutil.copyfile(os.path.join(
            model_dir, f"gemma-{variant}.gguf"), gguf_model)
    else:
...
def convert_yaml_to_json_config(yamlconfig, jsonconfig):
    ...
    """
    print(
        f"Converting yaml config from {yamlconfig} to json config {jsonconfig}")
